...
function checkEnv() {
  return {
    ...
    // gh 用户级安装可选的下载镜像（Linux/macOS 时非空；UI 据此渲染「下载源」下拉）。
    ghMirrors: IS_WIN ? [] : GH_MIRRORS,
  }
}
  ...
  const routes = {
    ...
    '/install-command': async (req, res) => {
      ...
      await handle(req, res, async (req) => {
        const body = JSON.parse((await readBody(req)) || '{}')
        const cmd = installCommand(String(body.tool || '').trim(), body.source)
        return { ok: true, tool: body.tool, command: cmd ? cmd.label : null }
      })
    },
