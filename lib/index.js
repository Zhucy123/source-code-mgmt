...
function providerOwner(provider) {
  if (provider === 'gitee') return giteeOwner()
  return ghOwner()
}
...
function repoStatus(dir, provider = 'github') {
  ...
  const changedFiles = statusLines.map((line) => {
    ...
    const type = /^\?\?/.test(code) ? 'untracked'
      ...
      : /^R/.test(code) ? 'renamed'
      : 'modified'
    // 内容 diff（供「查看改动」展开显示，删除行 - / 新增行 +）。
    // untracked 新文件没有 diff（git 不跟踪）；已跟踪的改动用 git diff -- path，
    // 为空再补 --cached（已暂存）的 diff。
    let diff = ''
    if (type !== 'untracked') {
      let r = run(GIT, ['-C', dir, 'diff', '--', path])
      if (r.ok && !r.stdout.trim()) {
        r = run(GIT, ['-C', dir, 'diff', '--cached', '--', path])
      }
      if (r.ok) diff = r.stdout
    }
    return { type, path, diff }
  })
  ...
  // measured against <providerRemote>/<branch>. A lightweight fetch keeps this
  // fresh; a failed fetch is tolerated (falls back to the last known tracking ref).
  let ahead = 0
  let behind = 0
