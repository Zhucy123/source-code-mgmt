 ...
 *
 * Port choice (user-selectable in the panel):
 *   - 443 (default): GitHub's official SSH-over-443 endpoint `ssh.github.com`.
 *     Keeps SSH usable on networks that BLOCK outbound port 22. Note this is
 *     NOT related to HTTP proxies: plain `ssh` ignores HTTP_PROXY/HTTPS_PROXY
 *     entirely, so having a proxy does not make port 22 reachable.
 *   - 22 (standard): plain `github.com:22` — the normal path when the network
 *     allows it (home/office/VPS). Gitee always uses its own host on both.
 * @param {string} [provider] - 'github' (default) | 'gitee'.
 * @param {number} [port] - 443 (default) | 22.
 */
function providerCfg(provider, port) {
  const name = provider === 'gitee' ? 'gitee' : 'github'
  // Only GitHub has a distinct 443 endpoint; Gitee's 443 host is the same host.
  const use443 = Number(port) === 22 ? false : true
  const host = name === 'gitee' ? 'gitee.com' : 'github.com'
  return {
    name, // 'github' | 'gitee'
    host, // Host key in ~/.ssh/config
    // Port 443 needs GitHub's alternate SSH endpoint; port 22 uses the plain host.
    hostname: name === 'gitee' ? 'gitee.com' : (use443 ? 'ssh.github.com' : 'github.com'),
    port: use443 ? 443 : 22,
    gitUser: 'git',
...
/** Normalize a user-supplied port choice to 443 or 22 (default 443). */
function normalizePort(port) {
  return Number(port) === 22 ? 22 : 443
}
...
function checkSsh() {
  ...
  // gitee 登录状态改由 Gitee 私人令牌（OpenAPI）判断，不再依赖 Gitee CLI 二进制。
  ...
  return {
    ...
    // Port actually pinned in each block (22 when absent). The panel shows this
    // instead of assuming 443, and uses it to preselect the port control.
    sshGitHubPort: (hostBlockSpan(configContent.split('\n'), 'github.com') || {}).port || null,
    sshGiteePort: (hostBlockSpan(configContent.split('\n'), 'gitee.com') || {}).port || null,
  }
}
...
/**
 * Write the SSH config block for github or gitee, on the chosen port.
 * Port 443 keeps SSH usable on networks that block port 22; port 22 is the
 * standard path. If an existing block for the same Host is present but pins a
 * DIFFERENT port, it is rewritten in place so switching actually takes effect.
 * @param {string} [provider] - 'github' (default) | 'gitee'.
 * @param {number} [port] - 443 (default) | 22.
 */
function writeSshConfig(provider, port) {
  const cfg = providerCfg(provider, port)
  try {
    mkdirSync(sshDir(), { recursive: true })
    let current = ''
    ...
    // Preserve the file's own line ending. Windows users very often have a CRLF
    // ~/.ssh/config; writing LF-only lines into it would flip parts of the file
    // and show up as a whole-file diff in their dotfiles repo.
    ...
    const hostRe = new RegExp('\\bHost\\s+' + cfg.host.replace(/\./g, '\\.') + '\\b')
    if (hostRe.test(current)) {
      // Already configured: keep it if the port already matches, else replace
      // just those lines (so switching 443 <-> 22 is a real change, not a no-op).
      // Split/join on the FILE's own EOL so the seams keep the original ending.
      const lines = current.split(eol)
      const span = hostBlockSpan(lines, cfg.host)
      if (span && span.port === cfg.port) {
        return { ok: true, alreadyConfigured: true, path: configPath(), provider: cfg.name, host: cfg.host, port: cfg.port }
      }
      const blockLines = block.split(eol)
      // `block` ends with eol, so drop the trailing empty element.
      if (blockLines.length && blockLines[blockLines.length - 1] === '') blockLines.pop()
      const next = span
        ? lines.slice(0, span.start).concat(blockLines, lines.slice(span.end)).join(eol)
        : current.replace(/[\r\n]*$/, '') + eol + block
      writeFileSync(configPath(), next, 'utf8')
      if (!IS_WIN) {
        try { chmodSync(configPath(), 0o600) } catch {}
      }
      return { ok: true, alreadyConfigured: false, updated: true, path: configPath(), provider: cfg.name, host: cfg.host, port: cfg.port, block }
    }

    writeFileSync(configPath(), current.replace(/[\r\n]*$/, '') + (current ? eol : '') + block, 'utf8')
    if (!IS_WIN) {
      try { chmodSync(configPath(), 0o600) } catch {}
    }
    return { ok: true, alreadyConfigured: false, path: configPath(), provider: cfg.name, host: cfg.host, port: cfg.port, block }
  } catch (error) {
...
/**
 * Find the `Host <host>` block in ssh-config lines and read its Port.
 * A block runs from its `Host` line up to (but excluding) the next `Host ` line
 * or EOF, so unrelated keywords inside it stay with it.
 * @returns {{ start: number, end: number, port: number } | null} line indices.
 */
 ...
 * The chosen port is applied explicitly (`-p`) so the test verifies the port the
 * user selected even before/without a matching ~/.ssh/config entry.
 * @param {string} [provider] - 'github' (default) | 'gitee'.
 * @param {number} [port] - 443 (default) | 22.
 */
function sshTest(provider, port) {
  const cfg = providerCfg(provider, port)
  const args = ['-o', 'StrictHostKeyChecking=accept-new', '-o', 'ConnectTimeout=20', '-T']
  // Port 443 needs GitHub's alternate endpoint; spelling it out avoids relying
  // on the config block being present.
  if (cfg.name !== 'gitee') args.push('-p', String(cfg.port), cfg.hostname === 'ssh.github.com' ? 'git@ssh.github.com' : cfg.testAddr)
  else args.push(cfg.testAddr)
  const r = run(SSH, args, { timeout: 45_000 })
  const blob = (r.stdout + '\n' + r.stderr)
...
function providerRemoteName(dir, provider) {
  const info = providerRemoteInfo(dir, provider)
  return info ? info.name : undefined
}

/**
 * Like {@link providerRemoteName} but also returns the URL, from the SAME single
 * `git remote -v` invocation — so the caller needs no extra `git remote get-url`
 * spawn just to display the remote.
 * @returns {{ name: string, url: string } | undefined}
 */
function providerRemoteInfo(dir, provider) {
  ...
  // Perf: one `git remote -v` instead of `git remote` + one `get-url` per remote.
  // Each spawn is ~35ms on Windows, and this runs on every panel load.
  const r = run(GIT, ['-C', dir, 'remote', '-v'])
  ...
  for (const line of r.stdout.split(/\r?\n/)) {
    ...
    if (want.test(url) && remoteOwner(url) === wantOwner) {
      if (name === 'origin') return { name, url }
      if (matched === undefined) matched = { name, url }
    }
  }
  return matched
}
...
/**
 * Decide "can this file be shown as text?" for MANY changed files at once.
 *
 * Perf: each `git` spawn costs ~35ms on Windows, and the old per-file fallback
 * ran `git diff --numstat -- <path>` once per unreadable file — an N+1 that made
 * panels with deleted files crawl (measured: 29 changed files = 1.2s). This
 * probes the working copy with a direct 8KB read (no process), and only when
 * files are genuinely unreadable does it make ONE batched `git diff --numstat`.
 *
 * Semantics are kept identical to the previous per-file version:
 *  - working copy exists  → NUL-byte sniff of its first 8KB (git's own heuristic);
 *  - otherwise            → viewable only if git reports a文本 numstat entry for
 *                           it (binary entries print "-\t-"; staged deletions
 *                           print nothing at all → NOT viewable, matching the
 *                           old single-path behaviour).
 *
 * @param {string} dir - repo root.
 * @param {Array<{path: string, type: string}>} files - changed files.
 * @returns {Map<string, boolean>} path -> viewable.
 */
function viewableMap(dir, files) {
  const result = new Map()
  const needGit = []
  for (const f of files) {
    const p = String(f.path || '')
    if (p === '') { result.set(p, false); continue }
    try {
      // Direct read: no process spawn. NUL byte => binary (same heuristic as git).
      const fd = openSync(join(dir, p), 'r')
      try {
        const buf = Buffer.alloc(8192)
        const n = readSync(fd, buf, 0, buf.length, 0)
        result.set(p, !buf.subarray(0, n).includes(0))
      } finally {
        closeSync(fd)
      }
    } catch {
      // Missing working copy (deleted, or staged-add then removed): defer to git.
      needGit.push(f)
    }
  }
  if (needGit.length === 0) return result

  // One batched numstat call per staging area instead of one call per path.
  const runBatch = (cached, list) => {
    if (list.length === 0) return
    const args = ['-C', dir, 'diff', ...(cached ? ['--cached'] : []), '--numstat', '--', ...list.map((f) => f.path)]
    const r = run(GIT, args)
    // "--numstat" prints "-\t-\t<path>" for binary entries. A path that appears
    // with real numbers is text; a path that does NOT appear at all (e.g. a
    // staged deletion, which numstat omits) is not previewable — that matches
    // the old per-path code, which treated "no line" as not viewable.
    ...
    for (const line of (r.stdout || '').split(/\r?\n/)) {
      ...
      const path = parseGitPath(parts.slice(2).join('\t'))
      if (!path) continue
      if (parts[0] === '-' && parts[1] === '-') binary.add(path)
      else textish.add(path)
    }
    for (const f of list) {
      result.set(f.path, !binary.has(f.path) && textish.has(f.path))
    }
  }
  runBatch(false, needGit.filter((f) => !(f.type === 'added' || f.type === 'renamed')))
  runBatch(true, needGit.filter((f) => f.type === 'added' || f.type === 'renamed'))
  return result
}
...
function repoStatus(dir, provider = 'github', full = false) {
  ...
  const parsedChanged = statusLines.map((line) => {
  ...
  // 性能：二进制判定**批量**做（直接读文件头 + 至多一次 git --numstat），
  // 而不是每个文件 spawn 一次 git —— Windows 上每次 git 进程约 36ms，
  // 改动多时旧写法会成为主要耗时。
  const viewable = viewableMap(dir, parsedChanged)
  const changedFiles = parsedChanged.map((f) => ({ ...f, viewable: viewable.get(f.path) === true }))
  ...
  // 名称与 URL 来自同一次 `git remote -v`（省掉再一次 git remote get-url 的进程开销）。
  const remoteInfo = providerRemoteInfo(dir, provider)
  const remoteName = remoteInfo ? remoteInfo.name : undefined
  const hasRemote = remoteName !== undefined
  const remoteUrl = remoteInfo ? remoteInfo.url : undefined
...
/**
 * 从 `gh --version` / `git --version` / release tag_name 等文本提取 x.y.z 版本号。
 * @param {string} text
 * @returns {string|null}
 */
...
function installCommandSystem(tool) {
  // 未知工具直接返回 null（Gitee CLI 已移除；避免 gitee 误落到 gh 的包名）。
  if (tool !== 'git' && tool !== 'gh' && tool !== 'ssh') return null
  if (IS_WIN) {
...
export function apply(ctx) {
  ...
  const routes = {
    ...
    // 检查 gh / git 是否有可用更新（本地版本 vs 官方最新 release）。
    ...
    '/write-config': async (req, res) => {
      ...
      await handle(req, res, async (req) => {
        const body = JSON.parse((await readBody(req)) || '{}')
        return { ok: true, ...writeSshConfig(body.provider, normalizePort(body.port)) }
      })
    },
    '/ssh-test': async (req, res) => {
      ...
      await handle(req, res, async (req) => {
        const body = JSON.parse((await readBody(req)) || '{}')
        return { ok: true, ...sshTest(body.provider, normalizePort(body.port)) }
      })
    },
...
export { parseVersion, compareVersions, checkGhUpdate, checkGitUpdate, checkEnv, installCommand, installCommandSystem, ghUserLevelScript, ghUserLevelSteps, ghReleaseAsset, ghDownloadUrl, normalizeGhSource, GH_MIRRORS, toolInstalled, canSudo, installToolFlow, npmStatus, npmLoginTerminal, npmRunStep, ghLoginTerminal, findTerminal }
