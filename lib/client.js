...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function RepoSection() {
			...
			const loadRepo = useCallback(async (d) => {
				setErr(null); setRepo(null);
				try {
			...
			return h(Box, { title: "③ 代码管理" },
				h("div", { style: { display: "flex", gap: 8, marginBottom: 10, alignItems: "center" } },
					...
					h(Field, { label: "改动", value: repo.dirty ? repo.dirtyCount + " 个文件" : "无" }),
					h(Field, { label: "同步" }, h("span", { style: { color: T.label, fontSize: 13 } },
				...
				(() => {
					...
					return h("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" } },
						btns,
						h(Btn, { label: "刷新状态", onClick: () => loadRepo(dir), disabled: busy })
					);
				})(),
