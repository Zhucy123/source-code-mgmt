	...
	factory: (require) => {
				...
				preloadPromise = (async () => {
					try {
						...
						if (def && def.dir) {
							...
							// 打开 DSH 时即做一次完整同步（full=1 会 git fetch 联网），
							// 这样点开「代码管理」面板直接显示最新状态，无需再等。
							cache.repo = await jget("/repo?dir=" + encodeURIComponent(def.dir) + "&full=1").catch(() => null);
						}
						cache.ready = true;
					} catch {
						/* 保持部分缓存 */
		...
		function RepoSection({ provider }) {
			...
			const loadRepo = useCallback(async (d, keepRepo, full) => {
				...
				// （切换目录 / 切换平台 / 刷新状态）自行清空。
				// full=true 时才让 host 端执行 git fetch 联网同步（用于「刷新状态」）；
				// 默认 false（快速模式）：跳过 fetch，秒出本地状态，避免每次切换/刷新都卡在网络。
					...
					return h("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" } },
						...
						h(Btn, {
							...
							onClick: async () => {
								...
								setRefreshing(true);
								try {
									// keepRepo=true：保留旧内容，避免刷新期间闪成「未加载文件夹」；
									// full=true：真正联网 git fetch 同步最新 ahead/behind。
									await loadRepo(dir, true, true);
								} finally {
									setRefreshing(false);
								}
							},
