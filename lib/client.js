 ...
 *   - branch B: not installed → the「代码管理」entry is registered in TWO places,
 *     both inside the conversation header, so it is always reachable without ever
 *     covering a control DSH itself draws:
 *       ① `conversation.session.header.utilities` — the right-aligned list that
 *          holds "Session log", so it sits beside it with the same pill look
 *          and 8px gap (rendered only while a session is active);
 *       ② `shell.overlay` — a pinned entry used while there is no active
 *          session. It sits at the TOP-RIGHT at the SAME HEIGHT as the entry
 *          above (top: 9px = the header's 10px pad + a 32px button centred in
 *          its 30px title row), plus --dsh-windows-titlebar-height on the
 *          desktop shell. It is inset `right: 48px`, so it starts clear of the
 *          empty-state header's right-sidebar expand button (which occupies
 *          right: 12..40px).
 *     The two are STRICTLY mutually exclusive (both keyed off the same
 *     `activeMainSession` signal), so exactly one「代码管理」is ever visible.
 *     Clicking either opens a RIGHT-side integrated panel (pushing #root left,
 *     dsh-better-sidebar style) holding the existing panel.
 *     If the `slots` service is unavailable, it falls back to a floating
 *     button opening the same right-side panel.
 *
 * The entry is deliberately NOT registered into DSH's own left sidebar
 * (`sidebar.footer.action`) — it would crowd that rail's own rows beside
 * "Settings". Seats owned by DSH (`conversation.session.header.corner` and
 * `.leading`, both `single`) are left alone too, as is `sidebar.panellist`
 * (its ids must be real main-panel ids).
...
"发布将使用官方源：": "Publishing uses the official registry: ",
  ...
  "登录失败": "Login failed",
  "登录失败：": "Login failed: ",
  // ② SSH：Gitee 登录（跟随平台切换；Gitee 认证走私人令牌，在 ② 配置）
  "Gitee 登录": "Gitee login",
  "Gitee 令牌": "Gitee token",
  "已配置私人令牌": "Personal token configured",
  "⚠️ 未配置": "⚠️ Not configured",
  "Gitee 认证走私人令牌，请在 ③ 代码管理配置": "Gitee authentication uses a personal token — configure it in ③ Code Management",
  "Gitee 私人令牌已配置": "Gitee personal token configured",
  "Gitee 私人令牌未配置 — 请在上方 ② SSH 密钥与连接里输入并保存（需 projects 权限，建仓/改可见性才可用）": "Gitee personal token not configured — enter and save it in ② SSH Key & Connection above (needs the projects scope for create/visibility)",
  "⚠️ 未配置 — Gitee 认证走私人令牌，在此输入并保存（需 projects 权限）": "⚠️ Not configured — Gitee authentication uses a personal token; enter and save it here (needs the projects scope)",
  "令牌只保存在本机 ~/.dsh/storages（0600），不会写进插件目录；需勾选个人令牌的 projects 权限。公钥需已上传到 Gitee（下方 SSH 测试）。": "The token is stored only in ~/.dsh/storages (0600), never in the plugin dir; the personal token needs the projects scope. The SSH public key must be uploaded to Gitee (SSH test below).",
  "导入失败": "Import failed",
  "导入失败：": "Import failed: ",
  "清除令牌失败": "Failed to clear the token",
  // ① 环境检查：检查更新
  "检查更新": "Check update",
  ...
  useEffect(() => {
    ...
    return off;
		...
		function EnvSection({ provider }) {
			// 环境检查固定检查 GitHub CLI (gh)（Gitee 已无 CLI 依赖，认证走私人令牌）。
			...
			// env 就绪后，若用户还没手动选过下载源，默认用第一个镜像（通常比官网快）。
			// 不依赖 installed：无论 gh 已安装（检查更新）还是未安装，更新/安装的下载源
			// 都是这个值；用独立 effect 避免 load() 异步时序导致「已安装点更新却落到官网」。
			useEffect(() => {
				if (!ghSourceTouched.current && env && Array.isArray(env.ghMirrors) && env.ghMirrors.length > 0) {
					setGhSource(env.ghMirrors[0].id);
				}
			}, [env]);
			...
			// 检查 gh / gitee / git 更新：host 对比本地与官方最新版本；有更新则询问并复用安装流（force）覆盖更新。
			const checkUpdate = useCallback(async (tool) => {
				...
				try {
					...
					if (!doIt) return;
					// 更新 = 按安装/升级逻辑重新执行（gh 走用户级安装脚本+所选下载源，gitee 走官方安装脚本，git 走系统包管理器升级）。
			...
			// gh 下载镜像列表（host 全平台下发；Windows 也支持，下载官方 msi 静默安装）。
			...
			const toolRow = (tool, label, installed, version) => {
				...
				const status = installed
					? h("span", { style: { display: "inline-flex", alignItems: "center", gap: 8 } },
						...
						// gh 已安装时提供「检查更新」；git 仅 Windows（Linux/macOS 的 git 是发行版/brew/
						// Xcode CLT 管理，版本与官方不同步——对比会误报，且随系统更新走，无需单独检查）。
						(tool === "gh" || (tool === "git" && winOs))
							? h("button", {
								type: "button",
				...
				// gh 时始终显示「下载源」下拉（官网直连可能极慢，镜像更快）。
				// 未安装：选源影响一键安装；已安装：选源影响「检查更新」触发的覆盖安装。
				// 三个平台（Windows/Linux/macOS）统一——已安装也要能选镜像再更新。
				const sourcePicker = (tool === "gh" && ghMirrors.length > 0)
					? h("div", { style: { marginTop: 4, display: "flex", alignItems: "center", gap: 6, fontSize: 12 } },
						...
						h("select", {
							...
							title: t("选择下载源：官网直连在国内网络可能很慢；镜像走加速代理（gh-proxy.com 实测最快）。安装与更新都用此源。"),
			...
			const cliOk = !!(env && env.gh && env.gh.installed);
		...
		// ---------- section 2: SSH ----------
		function SshSection({ provider, setProvider, onGiteeTokenChange }) {
			const [ssh, setSsh] = useState(() => cache.ssh);
			// env（gh 是否已安装；数据由 ① 预取/刷新维护，这里只读缓存）
			const [env, setEnv] = useState(() => cache.env);
			const [busy, setBusy] = useState(false);
			const [loginBusy, setLoginBusy] = useState(false);
			const [msg, setMsg] = useState(null);
			const [err, setErr] = useState(null);
			// Gitee 私人令牌（仅 gitee 模式显示）：在此处（②）直接输入/保存/清除。
			...
			// SSH 端口选择：443（默认，穿墙）或 22（标准）。
			//
			// 为什么要可选：443 是为了绕过**封锁 22 端口**的网络（GitHub 官方的
			// ssh.github.com:443 端点）—— 很多校园网/公司网/机房会封 22。而 22 是标准路径，
			// 家宽/办公室/VPS 通常直连就行。
			//
			// ⚠️ 注意一个常见误解：**这跟「有没有代理」无关**。普通 HTTP 代理只对
			// git-over-HTTPS / curl 生效；OpenSSH 完全无视 HTTP_PROXY / HTTPS_PROXY
			// （代理需要 ProxyCommand/nc 才走），所以「有代理」不等于「22 通了」。
			// 因此这里让用户自己按网络情况选，而不是由插件猜测。
			const [sshPort, setSshPort] = useState(443);
			// ② 默认折叠；在标题行也能切平台，切换时自动展开（因为要做后续配置）。
			const [collapsed, setCollapsed] = useState(true);

			// ---- Gitee 私人令牌：查询 / 保存 / 清除（/gitee-token 存储；③/④ 通过 onGiteeTokenChange 感知）----
			...
			// 加载 SSH 状态；Gitee 模式一并刷新令牌登录状态（② 内保存/清除后即时同步）。
			// 注意：load 必须先于所有依赖它的回调（ghLogin/act）定义。
			const load = useCallback(async () => {
				setErr(null);
				if (cache.ssh) setSsh(cache.ssh);
				try { setSsh(await refreshCache("ssh")); }
				catch (e) { setErr(String(e)); }
				// 同步 env 缓存（gh 是否已安装；env 数据由 ① 维护）
				if (cache.env) setEnv(cache.env);
				if (prov === "gitee") void loadGiteeStatus();
			}, [prov, loadGiteeStatus]);
			useEffect(() => { void preload().then(load); }, [load]);
			// 平台切到 gitee 时也刷新令牌状态（② 内保存后回到这里或切走再切回都同步）。
			useEffect(() => {
				if (prov === "gitee") void loadGiteeStatus();
			}, [prov, loadGiteeStatus]);
			...
			// 当前平台在 ~/.ssh/config 里实际写的端口（host 端解析；未配置为 null）。
			// 用于状态行显示「已配置(22)」这类真实信息，而不是写死 443。
			const configuredPort = ssh
				? (prov === "gitee" ? ssh.sshGiteePort : ssh.sshGitHubPort)
				: null;
			// 首次读到已有配置时，把端口选择器对齐到实际端口（只跟随一次，
			// 之后用户的显式选择优先，避免每次刷新都覆盖用户的选择）。
			const portSynced = useRef(false);
			useEffect(() => {
				if (portSynced.current) return;
				if (configuredPort === 22 || configuredPort === 443) {
					portSynced.current = true;
					setSshPort(configuredPort);
				}
			}, [configuredPort]);
			...
			// Gitee 登录/令牌配置行：私人令牌在 ② 这里直接输入、保存、清除。
			// Gitee 认证（建仓/改可见性等 OpenAPI）全靠这个令牌；SSH 只管已有仓库的 push/pull。
			const renderGiteeLogin = () => {
				if (giteeConfigured === null) return "…";
				return h("div", { style: { display: "flex", flexDirection: "column", gap: 6, width: "100%" } },
					giteeConfigured
						? h("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" } },
							h("span", { style: { color: T.success, fontSize: 13 } }, "✅ " + t("已配置私人令牌") + (giteeOwnerName ? t("（账号 ") + giteeOwnerName + "）" : "")),
						...
						: h("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
							h("div", { style: { fontSize: 12, color: T.secondary } }, t("⚠️ 未配置 — Gitee 认证走私人令牌，在此输入并保存（需 projects 权限）")),
							...
							h("div", { style: { fontSize: 11, color: T.secondary, lineHeight: 1.6 } },
								t("令牌只保存在本机 ~/.dsh/storages（0600），不会写进插件目录；需勾选个人令牌的 projects 权限。公钥需已上传到 Gitee（下方 SSH 测试）。"))
						),
					giteeTokenMsg ? h("div", { style: { color: T.success, fontSize: 12 } }, "✅ " + giteeTokenMsg) : null,
					giteeTokenErr ? h("div", { style: { color: T.danger, fontSize: 12 } }, "❌ " + giteeTokenErr) : null
				);
			};
			...
			return h(Box, {
				...
				h(Field, { label: prov === "gitee" ? t("Gitee 令牌") : t("GH 登录") },
					prov === "gitee"
						? renderGiteeLogin()
						: ssh
						? (ssh.ghLoggedIn
				...
				// SSH 配置状态 + 端口选择。端口显示**实际配置的端口**（host 端从 ~/.ssh/config
				// 解析出来的），不再无条件写「(443)」——因为端口现在由用户选。
				h(Field, { label: t("SSH 配置") }, h("span", { style: { display: "inline-flex", alignItems: "center", gap: 8, flexWrap: "wrap" } },
					ssh
						? (providerConfigured
							? h("span", { style: { color: T.label, fontSize: 13 } },
								"✅ " + providerHost + t(" 已配置") + (configuredPort ? "(" + configuredPort + ")" : ""))
							: h("span", { style: { color: T.warn, fontSize: 13 } }, t("⚠️ 未配置")))
						: h("span", { style: { color: T.label, fontSize: 13 } }, "…"),
					// 端口选择：443 穿墙 / 22 标准。切换后「配置 SSH」与「测试连接」都会用新端口。
					h("select", {
						value: String(sshPort),
						onChange: (e) => { setSshPort(Number(e.target.value)); setMsg(null); setErr(null); },
						title: t("443：绕过封锁 22 端口的网络（GitHub 走 ssh.github.com）；22：标准端口，家宽/办公室通常直连即可。注意：这与是否有 HTTP 代理无关——OpenSSH 不使用 HTTP_PROXY/HTTPS_PROXY。"),
						style: { font: "inherit", fontSize: 12, padding: "3px 6px", border: "1px solid " + T.border, borderRadius: 6, background: T.layer1, color: T.label, outline: "none", cursor: "pointer" },
					},
						h("option", { value: "443" }, t("443（穿墙）")),
						h("option", { value: "22" }, t("22（标准）")),
					)
				)),

				h("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 } },
					h(Btn, { label: t("生成密钥"), onClick: () => act("/gen-key", t("生成密钥")), disabled: busy }),
					h(Btn, { label: t("配置 SSH(config)"), onClick: () => act("/write-config", t("配置 SSH"), { provider: prov, port: sshPort }), disabled: busy }),
					h(Btn, { label: t("测试连接"), onClick: async () => {
						setBusy(true); setErr(null); setMsg(null);
						try {
							const r = await jpost("/ssh-test", { provider: prov, port: sshPort });
							if (r.connected) setMsg(t("SSH 连接成功（") + (prov === "gitee" ? "Gitee" : "GitHub") + "：" + String(sshPort) + "）：" + (r.account ? "Hi " + r.account : t("已认证")));
							else setErr(t("连接失败（端口 ") + String(sshPort) + t("），请确认密钥已上传到 ") + (prov === "gitee" ? "Gitee" : "GitHub") + (prov === "gitee" ? t(" 或已配置 Gitee 令牌") : t(" 或已登录 gh")) + t("；若是网络封锁 22 端口，请改用 443。"));
							void load();
		...
		// ---------- section 3: 代码管理 ----------
		function RepoSection({ provider, giteeTokenTick }) {
			...
			// Gitee 令牌状态（仅在 prov==='gitee' 时使用；令牌在 ② 配置，这里只读）
			...
			// ---- Gitee 令牌状态：只读（令牌在 ② 配置；③ 用于禁用「新建仓库」等按钮 + 显示账号）----
			...
			// ② 里保存/清除令牌后 giteeTokenTick +1，③ 这里也重新读取（令牌已配/清除即时生效）。
			useEffect(() => {
			...
			return h(Box, { title: t("③ 代码管理") },
				// Gitee 平台提示：令牌在 ② 配置，这里只读显示状态（未配置时提示去 ②）。
				prov === "gitee" ? h("div", { style: { border: "1px solid " + T.border, borderRadius: 10, padding: 10, marginBottom: 10, background: T.layer1, fontSize: 12 } },
					giteeConfigured === true
						? h("span", { style: { color: T.success } }, t("✅ Gitee 私人令牌已配置") + (giteeOwnerName ? t("（账号 ") + giteeOwnerName + "）" : ""))
						: h("span", { style: { color: T.warn } }, t("⚠️ Gitee 私人令牌未配置 — 请在上方 ② SSH 密钥与连接里输入并保存（需 projects 权限，建仓/改可见性才可用）"))
				) : null,
		...
		function CloneSection({ provider, setProvider, giteeTokenTick }) {
			...
			// 拉取远程仓库列表（含本地存在标记）。
			// giteeTokenTick：② 里保存/清除 Gitee 令牌后 +1，触发重新加载——
		...
		function ScmPanel({ onClose, variant }) {
			...
			return h("div", { style: panelStyle(variant), role: "dialog", "aria-modal": embedded ? undefined : "true", "aria-label": t("源代码管理") },
				...
				h(SshSection, { provider, setProvider, onGiteeTokenChange: () => setGiteeTokenTick((t) => t + 1) }),
				h("div", { style: { height: 12 } }),
				h(RepoSection, { provider, giteeTokenTick }),
				h("div", { style: { height: 12 } }),
		...
		/** 会话内 header utilities 里的「代码管理」按钮（①）。
		 *
		 *  和常驻入口（EmptyStateScmAction）**严格互斥**：只在主视图展示「非空白」会话时
		 *  渲染。DSH 在空白会话下会 `hideChrome` 整个 utilities 行，本来也不会显示；
		 *  但这里仍显式判一次，让两个入口的显隐条件只由一个信号（`activeMainSession`）
		 *  决定，避免任何一边的渲染时机差异导致同时冒出两个按钮。 */
		function HeaderScmEntry() {
			useSession();
			const snap = scmSessionList && typeof scmSessionList.getSnapshot === "function" ? scmSessionList.getSnapshot() : undefined;
			if (!snap) return null;
			const main = activeMainSession(snap);
			if (!main || main.blank !== false) return null;
			return h(HeaderScmAction);
		}

		/** 空态（无会话 / 新对话）时的常驻入口。
		 *
		 *  位置刻意避开 DSH 自带侧边栏按钮：DSH 的 `.header` 是 `padding: 10px 28px 0 20px`
		 *  + `.titleRow` 最小高 30px，自带侧栏开关（ui-sidebar 的 HeaderLeadingControls，
		 *  注册在 `conversation.session.header.leading`）就落在视口 y≈10..40 这一条带；
		 *  而空态下 `.headerCorner` 带 `margin-left:auto`，右侧同行同样被占。
		 *  旧实现用 `position: fixed; top: 8; right: 12`（按钮高 32 → y=8..40）正好压在这条带上，
		 *  这就是「代码管理」按钮与 DSH 自带侧栏按钮重叠的根因。
		 *
		 *  因此这里把常驻按钮从 header 那条带里挪出来：钉在 header 下沿（y=76px，即
		 *  `.header` 的 min-height）之下、贴右，横向也留出 DSH 的 28px 右内边距，
		 *  与自带侧栏按钮在纵向和横向都完全不相交。 */
		/** 当前主视图正在展示的会话（没有则 undefined）。
		 *
		 *  ⚠️ 踩过的坑：`sessions.list.getSnapshot()` **没有 `current` 字段**——它的形状是
		 *  `{ ids, byId, phase, subagentsByParent, jobsBySession }`（见
		 *  api/session-controller 的 SessionListState）。旧代码读 `snap.current` 永远拿到
		 *  undefined，于是 `hasActiveSession` 恒为 false，常驻按钮在活跃会话里也照样渲染，
		 *  结果「Session 日志」旁的按钮和常驻按钮**同时出现两个「代码管理」**。
		 *
		 *  DSH 自己的判定惯例是看 `retainedBy.mainView`（ui-layout/DocumentTitle、
		 *  ui-workspace/WorkspaceBrowser、ui-settings-general 等 5 处都这么写）——
		 *  保留计数 > 0 的那条就是主视图当前展示的会话。 */
		function activeMainSession(snap) {
			if (!snap || !snap.byId) return undefined;
			const rows = Object.keys(snap.byId).map((k) => snap.byId[k]).filter(Boolean);
			// 首选：主视图正在保留（展示）的那条。这是 DSH 自己的判定惯例，
			// 在 ui-layout/DocumentTitle、ui-workspace/WorkspaceBrowser、
			// ui-settings-general 等处都这么写。
			const retained = rows.find((s) => (s.retainedBy && s.retainedBy.mainView || 0) > 0);
			if (retained) return retained;
			// 兜底：若 DSH 版本没给 retainedBy（或字段改名），退回「非空白会话里最新的一条」。
			// 只影响「常驻按钮是否隐藏」这一个判断，拿不准时宁可认为有活跃会话
			// （此时只有 header 那个入口，不会出现两个按钮并存）。
			return rows.filter((s) => s.blank === false).sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0))[0];
		}

		/** 空态（无会话 / 新对话）时的常驻入口 —— 与「有会话时」那枚按钮**同一高度**、贴右上角。
		 *
		 *  纵向对齐（本次要解决的问题）：有会话时按钮挂在 header 的 utilities 行里，
		 *  那个行的几何是（ConversationRoot.module.css）
		 *    `.header`   padding-top: 10px
		 *    `.titleRow` min-height: 30px; align-items: center
		 *  ⇒ 标题行占 y = 10..40，**垂直中心 25px**。
		 *  按钮高 32px（headerBtnStyle 与这里都是 32），居中后它的 top = 25 - 16 = **9px**。
		 *  所以空态按钮的 top 也取 9px，两枚按钮就落在完全相同的高度上。
		 *
		 *  桌面壳（Windows）下 header 会被下方 titlebar 顶下去
		 *  （`--dsh-windows-titlebar-height`），于是这里用 header 自身的
		 *  `padding-top + (titleRow 高 - 按钮高)/2` 表达：
		 *    10px + (30px - 32px)/2 = 9px
		 *  普通浏览器下就是 9px，与有会话时逐像素一致。
		 *
		 *  横向：⚠️ 必须让开右上角已有的「右侧栏展开按钮」。
		 *  空态下会话 header 的 `.headerBlank .headerCorner` 带 `margin-left: auto`，
		 *  把 ui-sidebar-right 的 ExpandButton（28×28 圆钮）顶到最右；而
		 *    `.header`      padding-right: 28px
		 *    `.headerCorner` margin-right: -16px
		 *  所以那个圆钮占据视口右侧 **12px ~ 40px** 这一段。
		 *  早先写 `right: 12` 正好压在它上面（就是「叠在一起」的原因）。
		 *  现在用 `right: 48` = 40（圆钮左缘）+ 8（与 header 里其它控件的 8px 间距一致），
		 *  整个按钮落在圆钮左侧、互不相交。 */
		const SCM_TOPRIGHT_OFFSET = 48; // 40px 圆钮 + 8px gap
		const SCM_HEADER_PAD_TOP = 10;  // .header { padding-top: 10px }
		const SCM_TITLE_ROW_H = 30;     // .titleRow { min-height: 30px }
		const SCM_BUTTON_H = 32;        // headerBtnStyle 的 height
		/** 与「有会话时」完全同高：10 + (30 - 32)/2 = 9px。 */
		const SCM_BUTTON_TOP = SCM_HEADER_PAD_TOP + (SCM_TITLE_ROW_H - SCM_BUTTON_H) / 2;

		function EmptyStateScmAction() {
			useSession();
			const snap = scmSessionList && typeof scmSessionList.getSnapshot === "function" ? scmSessionList.getSnapshot() : undefined;
			// 尚未捕获到 sessions 时先不渲染：避免刷新瞬间（captureSessions 的 1.2s 延迟窗口内）
			// 因 scmSessionList 为 null 而错误点亮常驻按钮。
			if (!snap) return null;
			const main = activeMainSession(snap);
			// 主视图正展示「非空白」会话 → header utilities 里已有「代码管理」按钮；
			// 这里必须隐藏常驻按钮，否则会出现两个。
			// 空白（新对话/欢迎）或根本没有会话 → 显示常驻按钮。
			const hasActiveSession = !!main && main.blank === false;
			if (hasActiveSession) return null;
			return h("div", {
				style: {
					// 与有会话时的按钮同一高度（顶部 9px）；桌面壳里再叠加标题栏高度，
					// 这样两种形态的按钮在同一水平线上、切换时不跳。
					position: "fixed",
					top: "calc(var(--dsh-windows-titlebar-height, 0px) + " + SCM_BUTTON_TOP + "px)",
					right: SCM_TOPRIGHT_OFFSET,
		...
		function apply(ctx) {
			...
			// 分支 B：此刻未检测到 dsh-better-sidebar —— 注册两处「代码管理」入口（都在 header 里）：
			//   ① 有会话时：放进 conversation.session.header.utilities（Session 日志旁的右对齐列表）；
			//   ② 无会话空态时：放进常驻的 shell.overlay 槽，钉在 header **下方**（EmptyStateScmAction
			//      内部按「是否有当前会话」决定显隐，有会话时返回 null，避免与 ① 重复）。
			//      位置刻意避开 DSH 自带侧栏按钮所在的那条带（y≈10..40），详见 EmptyStateScmAction 注释。
			//   ①② 严格互斥（同一个 activeMainSession 信号判定），任何时刻只有一个「代码管理」可见。
			...
			if (slots && typeof slots.inject === "function") {
				ctx.effect(() => {
					const teardowns = [];
					// ① 会话内：Session 日志旁。用自门控的 HeaderScmEntry —— 只在主视图展示
					//    「非空白」会话时渲染，与 ② 常驻入口严格互斥（不会同时出现两个按钮）。
					teardowns.push(slots.inject('conversation.session.header.utilities', () => slots.register({
						name: 'conversation.session.header.utilities',
						id: 'source-code-mgmt',
						order: 200,
					}, HeaderScmEntry)));
					// ② 空态：常驻入口（钉在 header 下方，让开自带侧栏按钮）
					teardowns.push(slots.inject('shell.overlay', () => slots.register({
						name: 'shell.overlay',
						id: 'source-code-mgmt-empty',
						order: 300,
					}, EmptyStateScmAction)));
					// 刻意**不**注册进 DSH 自带侧边栏底部（`sidebar.footer.action`）：
					// 用户明确要求不要把入口放到左边栏（那里会和自带侧栏项混在一起，
					// 挤在「设置」上方）。入口只保留 header 两处：① 会话内 Session 日志旁、
					// ② 空态 header 下方常驻。
