								...
								detail === "changes" ? (
									repo && repo.changedFiles && repo.changedFiles.length > 0
										? repo.changedFiles.map((f, i) => {
											const hasDiff = !!(f.diff && f.diff.trim());
											...
											return h("div", { key: i },
												h("div", {
													...
													title: hasDiff ? (open ? "点击收起" : "点击查看改动内容") : "暂无内容 diff",
													onClick: hasDiff ? () => {
														setExpandedDiffs(prev => { const n = new Set(prev); if (n.has(i)) n.delete(i); else n.add(i); return n; });
													} : undefined,
												},
													...
													hasDiff ? h("span", { style: { color: T.brand, fontSize: 11, flexShrink: 0 } }, open ? "▾ 收起" : "▸ 查看")
														: null
												),
												open && hasDiff ? renderDiff(f.diff) : null
											);
										})
